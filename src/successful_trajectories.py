...
class WebVoyagerToQSFTMapper:
    ...
    def map_all_instances(self) -> List[QSFTTransition]:
        ...
        for instance_dir in instance_dirs:
            ...
            try:
                ...
                # Check if this instance was successful (has at least one transition with task_completed=True)
                instance_successful = any(t.task_completed for t in transitions)
                
                if instance_successful:
                    all_transitions.extend(transitions)
                    successful_instances.append(instance_id)
                    print(f"Processed {instance_id}: {len(transitions)} transitions (SUCCESS)")
                else:
                    failed_instances.append(instance_id)
                    print(f"Skipped {instance_id}: {len(transitions)} transitions (FAILED)")
        ...
        print(f"Total transitions from successful instances: {len(all_transitions)}")
    ...
    def generate_statistics(self, transitions: List[QSFTTransition]) -> Dict[str, Any]:
        """Generate statistics about the dataset (only successful trajectories)."""
        if not transitions:
            return {}
        ...
        return {
            ...
            "dataset_type": "successful_trajectories_only"
        }

def main():
    instances_dir = ".data/webvoyager-nnetnav/instances"
    output_path = "webvoyager_successful_qsft_dataset.json"
    
    mapper = WebVoyagerToQSFTMapper(instances_dir)
    
    print("Processing instances and filtering for successful trajectories only...")
    ...
    print("\nDataset Statistics (Successful Trajectories Only):")
    ...
    if transitions:
        print("\nSample transitions from successful trajectories:")
