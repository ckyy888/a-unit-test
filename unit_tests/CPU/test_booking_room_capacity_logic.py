...
class BookingRoomCapacityValidator:
    ...
    def log_occupancy_change(
        self, change_type: str, occupancy_data: Dict
    ) -> None:
        ...
        if change_type == "ADULT_DECREMENT":
            ...
            print(
                f"Occupancy changes: Adults reduced to {occupancy_data.get('adult_count', 'unknown')}"
            )
        ...
        if occupancy_data.get("capacity_valid"):
            ...
            print(
                "Capacity validation: Room becomes bookable (capacity ≤ max)"
            )
    ...
    def validate_capacity_logic_result(self) -> Dict[str, Any]:
        ...
        if self.adult_decrements < 2:
            validation_result["errors"].append(
                f"Need 2 adult decrements, got {self.adult_decrements}"
            )
        elif self.adult_decrements > 2:
            validation_result["warnings"].append(
                f"More decrements than needed: {self.adult_decrements}"
            )

        # Check if capacity warning was cleared
        if not self.capacity_warning_cleared:
            validation_result["errors"].append(
                "Capacity warning was not cleared"
            )
        ...
        validation_result["overall_valid"] = (
            validation_result["adult_decrements_executed"]
            and validation_result["capacity_warning_cleared"]
            and validation_result["room_bookable"]
...
class BookingRoomCapacityTestEnvironment:
    ...
    def __init__(
        ...
        self.url = self.config_loader.get("test_urls.booking_room_capacity") or url
        if not self.url:
            raise ValueError("Missing 'test_urls.booking_room_capacity' entry in config.json")
        ...
        self.booking_elements_config = self.config_loader.get_section(
            "booking_room_capacity_elements"
        )
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            self.test_state["occupancy_selector_open"] = element_detection.get(
                "adult_decrement", {}
            ).get("found", False)
            self.test_state["capacity_warning_visible"] = capacity_warning.get(
                "found", False
            )

            self.logger.info(
                f"Booking room capacity test initialized: {self.url}"
            )
            return setup_result
    ...
    async def _get_screenshot(self) -> str:
        ...
        try:
            ...
            try:
                ...
                d.text((100, 100), "Booking capacity (mock)", fill="black")
                buf = io.BytesIO()
    ...
    async def _check_capacity_warning(self) -> Dict:
        ...
        warning_config = self.booking_elements_config.get(
            "capacity_warning", {}
        )
        ...
        for selector in warning_config.get("selectors", []):
            try:
                ...
                self.logger.warning(
                    f"Failed to check warning selector {selector}: {e}"
                )
                continue
    ...
    async def _get_accessibility_tree(self) -> Dict:
        """Retrieve accessibility tree and nodes info for element-id mapping."""
        try:
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            # Use stored metadata if provided to map ID->coords consistently
            ...
            if action_type == 6:  # CLICK action
                if "decrement" in element_name or "minus" in element_name:
                    if (
                        occupancy_state.get("adult_count", 0)
                        < self.test_state["adult_count"]
                    ):
                        evaluation["valid"] = True
                        evaluation[
                            "feedback"
                        ] = f'Adult decrement executed - count reduced to {occupancy_state["adult_count"]}'

                        # Update internal state
                        self.test_state["adult_count"] = occupancy_state[
                            "adult_count"
                        ]

                        # Log the occupancy change
                        self.validator.log_occupancy_change(
                            "ADULT_DECREMENT",
                            {
                                "adult_count": occupancy_state["adult_count"],
                                "capacity_valid": occupancy_state.get(
                                    "capacity_valid", False
                                ),
                                "warning_cleared": not occupancy_state.get(
                        ...
                        if occupancy_state["adult_count"] <= 2:
                            ...
                            evaluation[
                                "validation_result"
                            ] = self.validator.validate_capacity_logic_result()
                        else:
                            evaluation[
                                "expected_next"
                            ] = "Continue clicking adult decrement to reach 2 adults maximum"
                    else:
                        evaluation[
                            "feedback"
                        ] = "Adult decrement clicked but count did not decrease"
                else:
                    evaluation[
                        "feedback"
                    ] = f"Clicked wrong element - expected adult decrement button, got: {element_name}"
            else:
                evaluation[
                    "feedback"
    ...
    async def _execute_action_in_browser(self, action: Dict, obs_metadata: Optional[Dict] = None) -> Dict:
        ...
        if action_type == 6:  # CLICK action
            # Prefer convert_action for element-id mapping
            try:
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            "capacity_warning_visible": self.test_state[
                "capacity_warning_visible"
            ],
...
async def test_booking_room_capacity_logic(
    url: str,
    ...
    """
    test_env = BookingRoomCapacityTestEnvironment(
...
def test_booking_room_capacity_logic_sync(
    url: str,
    run_agent_func: Callable,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
    ...
    return asyncio.run(
        test_booking_room_capacity_logic(
...
def create_booking_room_capacity_test(
    url: str,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
) -> BookingRoomCapacityTestEnvironment:
