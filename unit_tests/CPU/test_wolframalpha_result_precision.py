...
class WolframAlphaResultPrecisionTest:
    def __init__(self):
        self.url = "https://example.com/wolframalpha/query/result"
        self.first_task_description = (
            """
            Agent 1: The calculation result you see is displayed as "42.7".
            Task: Report the final rounded answer to one decimal place.
            """
        )
        self.second_task_description = (
            """
            Agent 2: The calculation result you see is displayed as "42.6666...".
            Task: Report the same rounded final answer to one decimal place.
            """
        )

    def evaluate_answer(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        final_url = agent_result.get("final_url", "")
        success_prefix = "https://example.com/wolframalpha/query/answer"
        passed = final_url.startswith(success_prefix)
        return {
            "passed": passed,
            "reason": (
                "Reported rounded answer" if passed else f"Unexpected final URL: {final_url}"
            ),
...
async def test_wolframalpha_result_precision():
    test_runner = WolframAlphaResultPrecisionTest()
    results = await test_runner.run_both_tests(
        test_runner.first_task_description, test_runner.second_task_description
    )

    print("\n" + "=" * 50)
    print("WOLFRAMALPHA RESULT PRECISION TEST RESULTS")
    print("=" * 50)
...
async def main():
    return await test_wolframalpha_result_precision()
