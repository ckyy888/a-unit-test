...
class AmazonVariantValidator:
    ...
    def log_variant_selection(self, selection_type: str, variant_data: Dict) -> None:
        ...
        if selection_type == 'SIZE_13_INCH':
            ...
            print(f"Variant selections: Size changed to {variant_data.get('size', '13 inch')}")
        elif selection_type == 'COLOR_AVAILABLE':
            self.color_selected = True
            print(f"Variant selections: Color changed to {variant_data.get('color', 'available option')}")
        ...
        if variant_data.get('price_updated'):
            ...
            print(f"Variant selections: Price updated to {variant_data.get('new_price', 'updated price')}")
    ...
    def validate_variant_selection_result(self) -> Dict[str, Any]:
        ...
        if not self.color_selected:
            validation_result['errors'].append("Available color was not selected")
        
        # Check if Add to Cart became enabled
        if not self.add_to_cart_enabled:
            validation_result['errors'].append("Add to Cart button did not become enabled")
        
        # Check if price was updated
        if not self.price_updated:
            validation_result['warnings'].append("Price was not updated after variant selection")
        
        # Determine if valid combination was found
        validation_result['valid_combination_found'] = (
            self.size_selected and 
            self.color_selected and 
            self.add_to_cart_enabled
        )
        
        # Overall validation status
        validation_result['overall_valid'] = (
            validation_result['valid_combination_found'] and
...
class AmazonVariantTestEnvironment:
    ...
    def __init__(self, url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None):
        ...
        self.amazon_elements_config = self.config_loader.get_section('amazon_variant_elements')
        ...
        try:
            ...
            setup_result = {
                ...
                'validation_rules': {
                    ...
                    'required_sequence': ['CLICK(13" size)', 'CLICK(available_color)'],
            ...
            self.logger.info(f"Amazon variant selection test initialized: {self.url}")
            return setup_result
        ...
        try:
            ...
            if action_type == 6:  # CLICK action
                if '13' in element_name and 'inch' in element_name:
                    ...
                    evaluation['feedback'] = '13-inch size selected successfully'
                    self.test_state['current_size'] = '13_inch'
                    self.test_state['size_changed'] = True
                    
                    # Log the variant selection
                    self.validator.log_variant_selection('SIZE_13_INCH', {
                        'size': '13 inch',
                        'available': True,
                        'add_to_cart_enabled': variant_state.get('add_to_cart_enabled', False),
                        'price_updated': bool(variant_state.get('price'))
                    })
                    
                    if not self.test_state['color_changed']:
                        evaluation['expected_next'] = 'Select an available color option'
                    else:
                        ...
                        if variant_state.get('add_to_cart_enabled'):
                            ...
                            evaluation['validation_result'] = self.validator.validate_variant_selection_result()
                
                elif 'color' in element_name or 'available' in element_name:
                    # Color selection
                    evaluation['valid'] = True
                    evaluation['feedback'] = 'Available color selected successfully'
                    self.test_state['current_color'] = 'available'
                    self.test_state['color_changed'] = True
                    
                    # Log the variant selection
                    self.validator.log_variant_selection('COLOR_AVAILABLE', {
                    ...
                    if not self.test_state['size_changed']:
                        evaluation['expected_next'] = 'Select 13-inch size option'
                    else:
                        # Both size and color selected, check if Add to Cart is enabled
                        if variant_state.get('add_to_cart_enabled'):
                            evaluation['test_completed'] = True
                    ...
                    evaluation['feedback'] = f'Clicked wrong element - expected size or color option, got: {element_name}'
            else:
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            'test_completed': self.test_state['size_changed'] and self.test_state['color_changed'],
...
async def test_amazon_variant_selection_logic(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """
...
def test_amazon_variant_selection_logic_sync(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """Synchronous wrapper for the async test function."""
    return asyncio.run(test_amazon_variant_selection_logic(url, run_agent_func, backend_url, config_path))


# Convenience function for quick integration
def create_amazon_variant_selection_test(url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> AmazonVariantTestEnvironment:
