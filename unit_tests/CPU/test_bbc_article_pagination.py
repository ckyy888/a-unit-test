...
class BBCPaginationValidator:
    ...
    def log_page_transition(self, transition_type: str, page_data: Dict) -> None:
        ...
        if transition_type == 'SCROLL_DOWN':
            ...
            print(f"Page transitions: Scrolled down to access pagination controls")
        elif transition_type == 'NEXT_PAGE_CLICKED':
            self.next_page_clicked = True
            print(f"Page transitions: Next page button clicked, navigating to page {page_data.get('target_page', '2')}")
            ...
            print(f"Page transitions: URL contains correct page parameter - {page_data.get('url_parameter', 'page=2')}")
    ...
    def validate_pagination_result(self) -> Dict[str, Any]:
        ...
        if not self.scrolled_down:
            validation_result['errors'].append("Did not scroll down to access pagination controls")
        
        # Check if next page was clicked
        if not self.next_page_clicked:
            validation_result['errors'].append("Next page button was not clicked")
        ...
        if not self.url_parameter_correct:
            validation_result['warnings'].append("URL does not contain correct page parameter")
        
        # Determine if pagination was successful
        validation_result['pagination_successful'] = (
            self.scrolled_down and 
            self.next_page_clicked and 
            self.page_2_content_loaded
        )
        
        # Overall validation status
        validation_result['overall_valid'] = (
            validation_result['pagination_successful'] and
...
class BBCPaginationTestEnvironment:
    ...
    def __init__(self, url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None):
        ...
        self.bbc_elements_config = self.config_loader.get_section('bbc_article_elements')
        ...
        try:
            ...
            setup_result = {
                ...
                'validation_rules': {
                    ...
                    'required_sequence': ['SCROLL_DOWN', 'CLICK(next_page)', 'verify content change'],
            ...
            self.logger.info(f"BBC article pagination test initialized: {self.url}")
            return setup_result
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            if action_type == 7:  # SCROLL_DOWN action
                ...
                evaluation['feedback'] = 'Scrolled down to access pagination controls'
                self.test_state['scrolled_to_bottom'] = True
                
                # Log the scroll action
                self.validator.log_page_transition('SCROLL_DOWN', {
                ...
                if 'next' in element_name or 'page' in element_name:
                    ...
                    evaluation['valid'] = True
                    evaluation['feedback'] = 'Next page button clicked successfully'
                    self.test_state['current_page'] = 2
                    
                    # Log the page transition
                    self.validator.log_page_transition('NEXT_PAGE_CLICKED', {
                        'target_page': 2,
                        'from_page': 1
                    })
                    ...
                    content_changed = await self._verify_content_change(page_info, updated_page_info)
                    if content_changed:
                        self.test_state['page_content_changed'] = True
                        
                        # Log successful content load
                        self.validator.log_page_transition('PAGE_2_CONTENT_LOADED', {
                            'new_content_length': updated_page_info.get('article_content_length', 0),
                            'page_2_loaded': True
                        })
                        
                        # Check URL parameter
                        if self._check_url_parameter(updated_page_info):
                            self.validator.log_page_transition('URL_PARAMETER_VERIFIED', {
                                'url_parameter': updated_page_info.get('page_parameter') or 'page=2',
                                'url': updated_page_info.get('current_url', '')
                            })
                        
                        evaluation['test_completed'] = True
                        evaluation['validation_result'] = self.validator.validate_pagination_result()
                    else:
                        evaluation['feedback'] = 'Next page clicked but content did not change'
                else:
                    evaluation['feedback'] = f'Clicked wrong element - expected next page button, got: {element_name}'
            else:
                evaluation['feedback'] = f'Unexpected action type {action_type} - expected SCROLL_DOWN (7) or CLICK (6)'
    ...
    async def _verify_content_change(self, before_info: Dict, after_info: Dict) -> bool:
        ...
        if abs(before_length - after_length) > 100:  # Significant content change
            return True
...
async def test_bbc_article_pagination(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """
...
def test_bbc_article_pagination_sync(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """Synchronous wrapper for the async test function."""
    return asyncio.run(test_bbc_article_pagination(url, run_agent_func, backend_url, config_path))


# Convenience function for quick integration
def create_bbc_article_pagination_test(url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> BBCPaginationTestEnvironment:
