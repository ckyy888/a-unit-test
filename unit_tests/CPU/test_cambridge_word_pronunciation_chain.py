...
class CambridgePronunciationValidator:
    ...
    def log_audio_playback_sequence(self, playback_type: str, audio_data: Dict) -> None:
        ...
        if playback_type == 'US_PRONUNCIATION_PLAYED':
            ...
            print(f"Audio playback sequence: US pronunciation played for comparison")
        elif playback_type == 'UK_PRONUNCIATION_PLAYED':
            self.uk_pronunciation_played = True
            print(f"Audio playback sequence: UK pronunciation played for comparison")
        elif playback_type == 'AUDIO_PLAYBACK_COMPLETED':
            self.both_pronunciations_compared = True
            print(f"Audio playback sequence: Both pronunciation flags marked as played")
    ...
    def validate_pronunciation_comparison_result(self) -> Dict[str, Any]:
        ...
        if not self.us_pronunciation_played:
            validation_result['errors'].append("US pronunciation was not played")
        
        # Check if UK pronunciation was played
        if not self.uk_pronunciation_played:
            validation_result['errors'].append("UK pronunciation was not played")
        
        # Check if both pronunciations were compared
        if not self.both_pronunciations_compared:
            validation_result['errors'].append("Both pronunciations were not compared")
        
        # Determine if pronunciation comparison was successful
        validation_result['pronunciation_comparison_successful'] = (
            self.us_pronunciation_played and 
            self.uk_pronunciation_played and 
            self.both_pronunciations_compared
        )
        
        # Overall validation status
        validation_result['overall_valid'] = (
            validation_result['pronunciation_comparison_successful'] and
...
class CambridgePronunciationTestEnvironment:
    ...
    def __init__(self, url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None):
        ...
        self.cambridge_elements_config = self.config_loader.get_section('cambridge_elements')
        ...
        try:
            ...
            setup_result = {
                ...
                'validation_rules': {
                    ...
                    'required_sequence': ['CLICK(US_pronunciation)', 'wait for audio', 'CLICK(UK_pronunciation)'],
            ...
            self.logger.info(f"Cambridge pronunciation comparison test initialized: {self.url}")
            return setup_result
        ...
        try:
            ...
            if action_type == 6:  # CLICK action
                if 'us' in element_name and 'pronunciation' in element_name:
                    ...
                    evaluation['feedback'] = 'US pronunciation button clicked - audio playing'
                    self.test_state['us_pronunciation_played'] = True
                    
                    # Log the pronunciation action
                    self.validator.log_audio_playback_sequence('US_PRONUNCIATION_PLAYED', {
                        'region': 'US',
                        'audio_started': True,
                        'word': pronunciation_state.get('word_displayed', 'though')
                    })
                    
                    if not self.test_state['uk_pronunciation_played']:
                        evaluation['expected_next'] = 'Wait for audio to complete, then click UK pronunciation'
                    else:
                        # Both pronunciations played
                        self._complete_pronunciation_comparison()
                        evaluation['test_completed'] = True
                        evaluation['validation_result'] = self.validator.validate_pronunciation_comparison_result()
                
                elif 'uk' in element_name and 'pronunciation' in element_name:
                    # UK pronunciation clicked
                    evaluation['valid'] = True
                    evaluation['feedback'] = 'UK pronunciation button clicked - audio playing'
                    self.test_state['uk_pronunciation_played'] = True
                    
                    # Log the pronunciation action
                    self.validator.log_audio_playback_sequence('UK_PRONUNCIATION_PLAYED', {
                        'region': 'UK',
                        'audio_started': True,
                        'word': pronunciation_state.get('word_displayed', 'though')
                    })
                    ...
                    if not self.test_state['us_pronunciation_played']:
                        evaluation['expected_next'] = 'Click US pronunciation to complete comparison'
                    else:
                        # Both pronunciations played
                        self._complete_pronunciation_comparison()
                        evaluation['test_completed'] = True
                    ...
                    evaluation['feedback'] = f'Clicked wrong element - expected pronunciation button, got: {element_name}'
            else:
    ...
    def _complete_pronunciation_comparison(self) -> None:
        ...
        # Log the completion
        self.validator.log_audio_playback_sequence('AUDIO_PLAYBACK_COMPLETED', {
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            'max_steps': self.max_steps,
            'us_pronunciation_played': self.test_state['us_pronunciation_played'],
            'uk_pronunciation_played': self.test_state['uk_pronunciation_played'],
            'both_pronunciations_compared': self.test_state['both_pronunciations_compared'],
...
async def test_cambridge_word_pronunciation_chain(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """
    Test function for Cambridge Dictionary pronunciation comparison with real browser integration.
    """
    test_env = CambridgePronunciationTestEnvironment(url, backend_url, config_path)
...
def test_cambridge_word_pronunciation_chain_sync(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """Synchronous wrapper for the async test function."""
    return asyncio.run(test_cambridge_word_pronunciation_chain(url, run_agent_func, backend_url, config_path))


# Convenience function for quick integration
def create_cambridge_word_pronunciation_chain_test(url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> CambridgePronunciationTestEnvironment:
