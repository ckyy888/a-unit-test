...
class AllRecipesValidator:
    ...
    def log_sort_and_filter_application(
        self, action_type: str, filter_data: Dict
    ) -> None:
        ...
        if action_type == "SORT_DROPDOWN_CLICKED":
            ...
            print(
                f"Sort and filter applications: URL parameters verified - {filter_data.get('url_params', 'sort=rating&time=30min')}"
            )
    ...
    def validate_sort_filter_result(self) -> Dict[str, Any]:
        ...
        if not self.rating_option_selected:
            validation_result["errors"].append(
                "Sort by rating option was not selected"
            )

        # Check if time filter was applied
        if not self.time_filter_applied:
            validation_result["errors"].append(
                "Under 30 min time filter was not applied"
            )

        # Check if URL parameters are correct
        if not self.url_parameters_correct:
            validation_result["warnings"].append(
                "URL does not contain both sort=rating&time=30min parameters"
            )

        # Determine if sort and filter was successful
        validation_result["sort_filter_successful"] = (
            self.sort_dropdown_clicked
            and self.rating_option_selected
            and self.time_filter_applied
        )
...
class AllRecipesTestEnvironment:
    ...
    def __init__(
        ...
        self.url = self.config_loader.get("test_urls.allrecipes_sort_filter") or url
        if not self.url:
            raise ValueError("Missing 'test_urls.allrecipes_sort_filter' in config.json")
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            self.logger.info(
                f"AllRecipes sort then filter test initialized: {self.url}"
            )
            return setup_result
    ...
    async def _get_screenshot(self) -> str:
        ...
        try:
            ...
            try:
                ...
                d.text((100, 100), "AllRecipes (mock)", fill="black")
                buf = io.BytesIO()
        ...
        try:
            ...
            if action_type == 6:  # CLICK action
                if "sort" in element_name and "dropdown" in element_name:
                    ...
                    evaluation[
                        "feedback"
                    ] = "Sort dropdown opened successfully"
                    self.test_state["sort_dropdown_open"] = True

                    # Log the sort action
                    self.validator.log_sort_and_filter_application(
                        "SORT_DROPDOWN_CLICKED", {"dropdown_opened": True}
                    )

                    evaluation[
                        "expected_next"
                    ] = "Select rating option from dropdown"
                    ...
                    if self.test_state["sort_dropdown_open"]:
                        ...
                        evaluation[
                            "feedback"
                        ] = "Sort by rating option selected"
                        self.test_state["rating_sort_applied"] = True
                        self.test_state["current_sort"] = "rating"

                        # Log the rating selection
                        self.validator.log_sort_and_filter_application(
                            "RATING_OPTION_SELECTED",
                            {"sort_by": "rating", "sort_applied": True},
                        )

                        evaluation[
                            "expected_next"
                        ] = "Wait for results to update, then apply time filter"
                    else:
                        evaluation[
                            "feedback"
                        ] = "Rating option clicked but sort dropdown was not opened first"

                elif (
                    "time" in element_name
                    or "30" in element_name
                    or "filter" in element_name
                ):
                    # Time filter applied
                    if self.test_state["rating_sort_applied"]:
                        ...
                        evaluation[
                            "validation_result"
                        ] = self.validator.validate_sort_filter_result()
                    else:
                        evaluation[
                            "feedback"
                        ] = "Time filter clicked but rating sort was not applied first"

                else:
                    evaluation[
                        "feedback"
                    ] = f"Clicked wrong element - expected sort/filter control, got: {element_name}"
            else:
                evaluation[
                    "feedback"
            ...
            try:
                ...
                if "coordinates" in action:
                    ...
                    if "sort" in element_name and "dropdown" in element_name:
                        ...
                        coords = self.allrecipes_elements_config.get("sort_dropdown", {}).get("coordinates", {})
                        x, y = coords.get("x", 0.8), coords.get("y", 0.3)
    ...
    def _check_url_parameters(self, page_state: Dict) -> bool:
        ...
        has_time_filter = time_param and (
            "30" in str(time_param) or "min" in str(time_param).lower()
        )
...
async def test_allrecipes_sort_then_filter(
    url: str,
...
def test_allrecipes_sort_then_filter_sync(
    url: str,
    run_agent_func: Callable,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
    ...
    return asyncio.run(
        test_allrecipes_sort_then_filter(
...
def create_allrecipes_sort_then_filter_test(
    url: str,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
) -> AllRecipesTestEnvironment:
