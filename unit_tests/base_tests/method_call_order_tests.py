...
class CallTracker:
    ...
    def record_call(self, method_name: str, args: tuple = (), kwargs: dict = None):
...
class MethodCallOrderTestMixin:
    ...
    def assertMethodCallOrder(self, expected_order: List[str], call_tracker: CallTracker) -> None:
        """
        ...
        # Compare the orders
        self.assertEqual(len(expected_order), len(actual_order),
    ...
    def assertMethodCallSequence(self, expected_sequence: List[Dict[str, Any]], 
                               call_tracker: CallTracker) -> None:
        """
        ...
        self.assertEqual(len(expected_sequence), len(actual_calls),
                        f"Expected {len(expected_sequence)} method calls, got {len(actual_calls)}")
        
        for i, (expected, actual) in enumerate(zip(expected_sequence, actual_calls)):
            # Check method name
            ...
            if 'args' in expected:
                self.assertEqual(expected['args'], actual[1],
            ...
            if 'kwargs' in expected:
                self.assertEqual(expected['kwargs'], actual[2],
                               f"Method call {i} kwargs mismatch: expected {expected['kwargs']}, got {actual[2]}")
    
    def assertMethodCallCount(self, method_name: str, expected_count: int, 
                            call_tracker: CallTracker) -> None:
        """
        ...
        Args:
            ...
            call_tracker: CallTracker object that records method calls
        """
        actual_count = sum(1 for call in call_tracker.calls if call[0] == method_name)
        
        self.assertEqual(expected_count, actual_count,
                        f"Method '{method_name}' call count mismatch: expected {expected_count}, got {actual_count}")
    
    def assertMethodCallWithArgs(self, method_name: str, expected_args: tuple, 
                               expected_kwargs: dict, call_tracker: CallTracker) -> None:
        """
        ...
        for call in call_tracker.calls:
            if (call[0] == method_name and
                call[1] == expected_args and
                call[2] == expected_kwargs):
                method_called = True
                break
    ...
    def assertMethodCallContains(self, method_name: str, expected_args: tuple, 
                               call_tracker: CallTracker) -> None:
        """
            ...
            if call[0] == method_name:
                ...
                if len(actual_args) >= len(expected_args):
                    ...
                    for i, expected_arg in enumerate(expected_args):
                        if i >= len(actual_args) or actual_args[i] != expected_arg:
                            args_match = False
                            break
                    if args_match:
                        method_called = True
    ...
    def assertMethodCallBetween(self, method_name: str, start_method: str, end_method: str, 
                              call_tracker: CallTracker) -> None:
        """
        ...
        try:
            ...
            self.fail(f"Could not find required methods in call sequence: {str(e)}")
    
    def assertMethodCallNotBetween(self, method_name: str, start_method: str, end_method: str, 
                                 call_tracker: CallTracker) -> None:
        """
        ...
        try:
            ...
            for i in range(start_index + 1, end_index):
                if method_calls[i] == method_name:
                    self.fail(f"Method '{method_name}' was called between '{start_method}' and '{end_method}'")
        except ValueError as e:
            self.fail(f"Could not find required methods in call sequence: {str(e)}")
    
    def assertMethodCallFrequency(self, expected_frequency: Dict[str, int], 
                                call_tracker: CallTracker) -> None:
        """
        ...
        for call in call_tracker.calls:
            method_name = call[0]
            actual_frequency[method_name] = actual_frequency.get(method_name, 0) + 1
    ...
    def assertMethodCallArguments(self, method_name: str, call_index: int, 
                                expected_args: tuple, expected_kwargs: dict, 
                                call_tracker: CallTracker) -> None:
        """
        ...
        self.assertGreater(len(method_calls), call_index,
    ...
    def assertMethodCallReturnValue(self, method_name: str, call_index: int, 
                                  expected_return_value: Any, call_tracker: CallTracker) -> None:
        """
        ...
        target_call = method_calls[call_index]
        actual_return_value = target_call[0] # The method itself is the return value
    ...
    def assertMethodCallSideEffect(self, method_name: str, call_index: int, 
                                 expected_side_effect: Any, call_tracker: CallTracker) -> None:
        """
        ...
        target_call = method_calls[call_index]
        actual_side_effect = target_call[0] # The method itself is the side effect
        
        self.assertEqual(expected_side_effect, actual_side_effect,
                        f"Method '{method_name}' call {call_index} side effect mismatch: "
                        f"expected {expected_side_effect}, got {actual_side_effect}")
    
    def assertMethodCallException(self, method_name: str, call_index: int, 
                                expected_exception: Type[Exception], call_tracker: CallTracker) -> None:
        """
        ...
        if callable(actual_side_effect):
            ...
            try:
                actual_side_effect()
                self.fail(f"Method '{method_name}' call {call_index} did not raise an exception")
            ...
            self.fail(f"Method '{method_name}' call {call_index} has no side effect")
    
    def assertMethodCallTimeout(self, method_name: str, max_call_time: float, 
                              call_tracker: CallTracker) -> None:
        """
        ...
        self.assertLessEqual(call_time, max_call_time,
                           f"Method '{method_name}' call took {call_time:.3f}s, "
                           f"exceeded limit of {max_call_time}s")
    
    def assertMethodCallConcurrency(self, method_names: List[str], 
                                  call_tracker: CallTracker) -> None:
        """
        ...
        for method_name in method_names:
            self.assertIn(method_name, method_calls,
        ...
        self.assertGreaterEqual(len(method_calls), len(method_names),
                               f"Expected at least {len(method_names)} concurrent calls, "
                               f"but found {len(method_calls)}")
    
    def assertMethodCallDependency(self, dependent_method: str, prerequisite_method: str, 
                                 call_tracker: CallTracker) -> None:
        """
        ...
        try:
            ...
            self.assertLess(prerequisite_index, dependent_index,
                           f"Method '{dependent_method}' was called before its prerequisite '{prerequisite_method}'")
        except ValueError as e:
            self.fail(f"Could not find required methods in call sequence: {str(e)}")
    
    def assertMethodCallIsolation(self, method_name: str, 
                                isolated_methods: List[str], call_tracker: CallTracker) -> None:
        """
        ...
        target_indices = [i for i, method in enumerate(method_calls) if method == method_name]
        ...
        for target_index in target_indices:
            ...
            for isolated_method in isolated_methods:
                if isolated_method in method_calls:
                    isolated_indices = [i for i, method in enumerate(method_calls) if method == isolated_method]
                    for isolated_index in isolated_indices:
                        ...
                        if abs(target_index - isolated_index) <= 2:
                            self.fail(f"Method '{method_name}' was called concurrently with '{isolated_method}'")
    
    def assertMethodCallOrderInvariant(self, invariant_condition: Callable[[List[str]], bool], 
                                     call_tracker: CallTracker) -> None:
        """
        ...
        self.assertTrue(invariant_condition(method_calls),
