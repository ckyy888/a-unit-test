...
"""
from typing import Any, Callable, Type, Union, Optional, List


class ExceptionTestMixin:
    """Mixin for testing exception behavior in agent trajectories."""
    
    def assertRaises(self, exception: Type[Exception], callable: Callable, 
                    *args, **kwargs) -> None:
        """
        ...
        try:
            ...
            self.fail(f"Expected {exception.__name__} but got {type(e).__name__}: {str(e)}")
    
    def assertRaisesWithMessage(self, exception: Type[Exception], expected_message: str, 
                              callable: Callable, *args, **kwargs) -> None:
        """
        ...
        try:
            callable(*args, **kwargs)
            self.fail(f"Expected {exception.__name__} with message '{expected_message}' to be raised")
        except exception as e:
            if expected_message not in str(e):
                self.fail(f"Expected exception message to contain '{expected_message}', "
                         f"but got: '{str(e)}'")
        except Exception as e:
            self.fail(f"Expected {exception.__name__} but got {type(e).__name__}: {str(e)}")
    
    def assertRaisesRegex(self, exception: Type[Exception], expected_regex: str, 
                         callable: Callable, *args, **kwargs) -> None:
        """
        ...
        try:
            callable(*args, **kwargs)
            self.fail(f"Expected {exception.__name__} with message matching '{expected_regex}' to be raised")
        except exception as e:
            if not re.search(expected_regex, str(e)):
                self.fail(f"Expected exception message to match '{expected_regex}', "
                         f"but got: '{str(e)}'")
        except Exception as e:
            self.fail(f"Expected {exception.__name__} but got {type(e).__name__}: {str(e)}")
    ...
    def assertNoException(self, callable: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            self.fail(f"Expected no exception but got {type(e).__name__}: {str(e)}")
    
    def assertExceptionInTrajectory(self, exception_type: Type[Exception], 
        ...
        self.assertGreater(len(exceptions_found), 0,
                          f"No {exception_type.__name__} exceptions found in trajectory")
    
    def assertExceptionCount(self, expected_count: int, exception_type: Type[Exception], 
                           trajectory: List[Any]) -> None:
        """
    ...
    def assertExceptionHandled(self, exception_type: Type[Exception], 
                             trajectory: List[Any]) -> None:
        """
        ...
        for i, item in enumerate(trajectory):
            if isinstance(item, dict) and 'exception' in item:
                if isinstance(item['exception'], exception_type):
                    ...
                    for j in range(i + 1, min(i + 5, len(trajectory))):  # Look ahead 5 steps
                        next_item = trajectory[j]
                        if isinstance(next_item, dict) and 'action' in next_item:
        ...
        self.assertEqual(len(unhandled_exceptions), 0,
                        f"Found {len(unhandled_exceptions)} unhandled {exception_type.__name__} exceptions: "
                        f"{unhandled_exceptions}")
    
    def assertExceptionRecoveryTime(self, exception_type: Type[Exception], 
                                  max_recovery_time: float, trajectory: List[Any]) -> None:
        """
                ...
                if isinstance(item['exception'], exception_type):
                    ...
                    for j in range(i + 1, len(trajectory)):
                        ...
                        if isinstance(next_item, dict) and 'action' in next_item:
                            action = next_item['action']
                            if self._is_recovery_action(action):
                                if 'timestamp' in item and 'timestamp' in next_item:
                                    recovery_time = next_item['timestamp'] - item['timestamp']
                                break
                        elif hasattr(next_item, 'action'):
                            action = next_item.action
                            if self._is_recovery_action(action):
                                if hasattr(item, 'timestamp') and hasattr(next_item, 'timestamp'):
                                    recovery_time = next_item.timestamp - item.timestamp
                                break
        ...
        for recovery_time in recovery_times:
            self.assertLessEqual(recovery_time, max_recovery_time,
                               f"Recovery time {recovery_time}s exceeded limit of {max_recovery_time}s")
    
    def assertExceptionContext(self, exception_type: Type[Exception], 
                             expected_context: str, trajectory: List[Any]) -> None:
        """
        ...
        for item in trajectory:
            if isinstance(item, dict) and 'exception' in item:
                if isinstance(item['exception'], exception_type):
                    if 'context' in item and expected_context in item['context']:
                        context_found = True
                        break
            elif hasattr(item, 'exception'):
                if isinstance(item.exception, exception_type):
                    if hasattr(item, 'context') and expected_context in item.context:
                        context_found = True
                        break
    ...
    def assertExceptionSeverity(self, exception_type: Type[Exception], 
                              expected_severity: str, trajectory: List[Any]) -> None:
        """
            ...
            if isinstance(item, dict) and 'exception' in item:
                if isinstance(item['exception'], exception_type):
                    ...
                    if expected_severity in severity_indicators:
                        ...
                        if any(indicator in exception_str for indicator in indicators):
        ...
        self.fail(f"No {exception_type.__name__} exceptions found with severity '{expected_severity}'")
    
    def assertExceptionChain(self, exception_types: List[Type[Exception]], 
                           trajectory: List[Any]) -> None:
        """
        ...
        for i, expected_type in enumerate(exception_types):
            if i < len(found_exceptions):
                self.assertEqual(expected_type, found_exceptions[i],
                               f"Exception chain mismatch at position {i}: "
                               f"expected {expected_type.__name__}, got {found_exceptions[i].__name__}")
            else:
                self.fail(f"Exception chain too short: expected {len(exception_types)} exceptions, "
                         f"but found {len(found_exceptions)}")
    
    def assertExceptionSuppressed(self, exception_type: Type[Exception], 
                                trajectory: List[Any]) -> None:
        """
    ...
    def _is_recovery_action(self, action: Union[str, Any]) -> bool:
        ...
        recovery_indicators = [
            'retry', 'again', 'reload', 'refresh', 'restart',
            'fallback', 'alternative', 'backup', 'reset', 'clear',
            'skip', 'ignore', 'continue', 'wait', 'sleep', 'delay',
            'recover', 'restore', 'rollback', 'undo', 'revert'
        ]
        
        return any(indicator in action_str for indicator in recovery_indicators) 