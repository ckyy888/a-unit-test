...
from unit_tests.trajectory_convergence_tests import TrajectoryConvergenceTestMixin
...
class WebArenaBaseTest(unittest.TestCase,
    ...
    def assertTrajectoryComplete(self, min_steps: int = 6):
        ...
        self.assertGreaterEqual(len(self.trajectory), min_steps,
        ...
        for i, item in enumerate(self.trajectory):
            if i % 2 == 0:
                self.assertIn("observation", item, f"Expected observation at index {i}")
                self.assertIn("text", item["observation"], f"Observation at index {i} should have text")
            else:
                self.assertIn("action_type", item, f"Expected action at index {i}")

    def assertActionSequenceValid(self, expected_actions: List[int]):
        """Assert that trajectory contains expected action sequence."""
        actions = [item for item in self.trajectory if isinstance(item, dict) and "action_type" in item]
        ...
        for expected_type in expected_actions:
            try:
                ...
                self.fail(f"Missing action type {expected_type} after position {last_idx}")
    ...
    def assertObservationQuality(self, min_length: int = 100):
        ...
        for i, item in enumerate(self.trajectory):
            if isinstance(item, dict) and "observation" in item:
                ...
                self.assertGreaterEqual(len(obs_text), min_length,
                                      f"Observation {i} should have at least {min_length} characters")
                
                # Check accessibility tree structure
                self.assertIn("[", obs_text, f"Observation {i} should have element IDs")
                self.assertIn("]", obs_text, f"Observation {i} should have element IDs")

    def assertFinalAnswer(self, expected_content: str):
        """Assert that final answer contains expected content."""
        stop_actions = [item for item in self.trajectory 
                       if isinstance(item, dict) and item.get("action_type") == 17]
        
        self.assertEqual(1, len(stop_actions), "Should have exactly one stop action")
        stop_action = stop_actions[0]
        
        self.assertIn("answer", stop_action, "Stop action should have an answer field")
        answer = stop_action["answer"]
        self.assertIn(expected_content.lower(), answer.lower(), 
        ...
        for item in self.trajectory:
            if isinstance(item, dict):
                if "error" in str(item).lower() or \
                   (item.get("info", {}).get("fail_error", "") != "") or \
    ...
    def assertActionParameters(self, action_type: int, required_params: List[str]):
        """Assert that actions of given type have required parameters."""
        actions = [item for item in self.trajectory 
                  if isinstance(item, dict) and item.get("action_type") == action_type]
        
        for action in actions:
            for param in required_params:
                self.assertIn(param, action,
                            f"Action type {action_type} missing required parameter '{param}'")

    def assertStateTransitions(self, initial_state_pattern: str, final_state_pattern: str):
        ...
        self.assertGreaterEqual(len(pages), 2, "Should have at least initial and final states")
        self.assertIn(initial_state_pattern, str(pages[0]).lower(),
                     f"Initial state should contain '{initial_state_pattern}'")
        self.assertIn(final_state_pattern, str(pages[-1]).lower(),
    ...
    def assertScore(self, expected_score: float, tolerance: float = 0.01):
        ...
        self.assertIsInstance(actual_score, (int, float),
                            "Score should be a number")
        self.assertAlmostEqual(expected_score, actual_score, delta=tolerance,
