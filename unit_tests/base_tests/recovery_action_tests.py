...
class RecoveryActionTestMixin:
    ...
    def assertRecoveryAction (self, error_condition: str, expected_action: Union[str, Any], 
                           actual_action: Union[str, Any]) -> None:
        """
    ...
    def assertRecoveryActionInTrajectory(self, error_condition: str, 
        ...
        self.assertGreater(len(found_recovery_actions), 0,
                          f"No recovery actions found for error '{error_condition}'. "
                          f"Expected one of: {expected_recovery_actions}")
    
    def assertRecoverySequence(self, error_condition: str, 
        ...
        sequence_found = self._find_sequence_in_trajectory(expected_actions, actual_actions)
    ...
    def assertRecoveryTimeout(self, error_condition: str, max_recovery_time: float, 
        ...
        try:
            ...
            self.fail(f"Recovery function failed for error '{error_condition}': {str(e)}")
    
    def assertRecoveryRetryCount(self, error_condition: str, expected_retry_count: int, 
    ...
    def assertRecoveryStateRestoration(self, error_condition: str, 
    ...
    def assertRecoveryActionType(self, error_condition: str, 
        ...
        if expected_action_type in type_indicators:
            ...
            found_indicator = any(indicator in action_str for indicator in indicators)
            self.assertTrue(found_indicator,
                          f"Recovery action for error '{error_condition}' doesn't match expected type '{expected_action_type}'. "
                          f"Action: {actual_action}")
    
    def assertRecoveryStrategy(self, error_condition: str, 
        ...
        if expected_strategy == 'immediate':
            ...
            has_delays = any(any(indicator in action.lower() for indicator in delay_indicators) 
                           for action in recovery_actions)
            self.assertTrue(has_delays,
            ...
            self.assertGreater(len(recovery_actions), 1,
                             f"No progressive recovery strategy found for error '{error_condition}'")
    
    def assertRecoverySuccess(self, error_condition: str, 
        ...
        if isinstance(success_indicator, str):
            ...
            self.assertIn(success_indicator, trajectory_text,
    ...
    def assertRecoveryFailure(self, error_condition: str, 
        ...
        if isinstance(failure_indicator, str):
            ...
            trajectory_text = str(actual_trajectory)
            self.assertIn(failure_indicator, trajectory_text,
            ...
            self.assertTrue(failure,
                          f"Recovery did not fail for error '{error_condition}'")
    ...
    def _find_sequence_in_trajectory(self, expected_sequence: List[str], actual_actions: List[str]) -> bool:
        """
        ...
        for i in range(len(actual_actions) - len(expected_sequence) + 1):
            if actual_actions[i:i + len(expected_sequence)] == expected_sequence:
                return True
