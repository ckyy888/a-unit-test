...
class ActionSequenceTestMixin:
    ...
    def assertActionSequence(
        self,
        expected_sequence: List[Union[str, Any]],
        ...
        for i, (expected, actual) in enumerate(
            zip(expected_actions, actual_actions)
        ):
            self.assertEqual(
                expected,
                actual,
    ...
    def assertActionSequenceContains(
        self,
        expected_actions: List[Union[str, Any]],
        actual_trajectory: List[Any],
    ) -> None:
        """
        ...
        for expected_action in expected_actions:
            if isinstance(expected_action, str):
                expected_action = self._normalize_action_string(
                    expected_action
                )
            else:
    ...
    def assertActionSequencePrefix(
        self,
        expected_prefix: List[Union[str, Any]],
        actual_trajectory: List[Any],
    ) -> None:
        """
    ...
    def assertActionSequenceSuffix(
        self,
        expected_suffix: List[Union[str, Any]],
        actual_trajectory: List[Any],
    ) -> None:
        """
        ...
        for i, expected in enumerate(expected_actions):
            ...
            self.assertEqual(
                expected,
                actual_actions[actual_index],
                f"Suffix mismatch at position {i}: expected {expected}, got {actual_actions[actual_index]}",
            )

    def assertActionFrequency(
        self, expected_frequency: Dict[str, int], actual_trajectory: List[Any]
    ) -> None:
        """
        ...
        for action in actual_actions:
            ...
            actual_frequency[action_type] = (
                actual_frequency.get(action_type, 0) + 1
            )
    ...
    def assertActionPattern(
        self, expected_pattern: str, actual_trajectory: List[Any]
    ) -> None:
        """
    ...
    def assertActionBetween(
        ...
        try:
            ...
            self.fail(
                f"Could not find required actions in trajectory: {str(e)}"
            )

    def assertActionNotBetween(
        self,
        action_name: str,
        start_action: str,
        end_action: str,
        ...
        try:
            ...
            for i in range(start_index + 1, end_index):
                if actual_actions[i] == action_name:
                    self.fail(
                        f"Action '{action_name}' was called between '{start_action}' and '{end_action}'"
                    )
        except ValueError as e:
            self.fail(
                f"Could not find required actions in trajectory: {str(e)}"
