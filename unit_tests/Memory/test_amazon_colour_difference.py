...
class AmazonColourDifferenceTest:
    def __init__(self):
        self.url = "https://example.com/amazon/owala/bottle"
        self.first_task_description = (
            """
            Agent 1: Choose the correct colour variant "Owala" and proceed to purchase.
            """
        )
        self.second_task_description = (
            """
            Agent 2: Colour names are displayed differently, but select the same "Owala" colour and proceed.
            """
        )

    def evaluate_purchase(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        final_url = agent_result.get("final_url", "")
        success_prefix = "https://example.com/amazon/owala/confirmation"
        passed = final_url.startswith(success_prefix)
        return {
            "passed": passed,
            "reason": (
                "Correct colour selected and purchase continued"
                if passed
    ...
    async def run_first_test(self, task_description: str) -> Dict[str, Any]:
        ...
        evaluation = self.evaluate_purchase(result)
        return {
            "test_name": "first_test",
            "scenario": "colour_label_standard",
            "agent_result": result,
...
async def test_amazon_colour_difference():
    test_runner = AmazonColourDifferenceTest()
    results = await test_runner.run_both_tests(
        test_runner.first_task_description, test_runner.second_task_description
    )

    print("\n" + "=" * 50)
    print("AMAZON COLOUR DIFFERENCE TEST RESULTS")
    print("=" * 50)
...
async def main():
    return await test_amazon_colour_difference()
