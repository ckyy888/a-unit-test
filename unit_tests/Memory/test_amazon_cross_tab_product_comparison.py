...
class AmazonCrossTabProductComparisonTest:
    def __init__(self):
        self.url = "https://example.com/amazon/laptops/compare"
        self.first_task_description = (
            """
            Agent 1: Compare two laptops with both tabs open. Select the better option and proceed.
            """
        )
        self.second_task_description = (
            """
            Agent 2: First tab reloads when switching back. Retain specs from memory and select the same option.
            Proceed to purchase.
            """
        )
    ...
    def evaluate_selection(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        success_prefix = "https://example.com/amazon/laptops/selected"
        passed = final_url.startswith(success_prefix)
        return {
            "passed": passed,
            "reason": (
                "Selected laptop with retained comparison context"
                if passed
    ...
    async def run_second_test(self, task_description: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "scenario": "tab_reload_memory_needed",
            "agent_result": result,
            "evaluation": evaluation,
            "passed": evaluation["passed"],
        }
...
async def test_amazon_cross_tab_product_comparison():
    test_runner = AmazonCrossTabProductComparisonTest()
    results = await test_runner.run_both_tests(
        test_runner.first_task_description, test_runner.second_task_description
    )

    print("\n" + "=" * 50)
    print("AMAZON CROSS TAB PRODUCT COMPARISON TEST RESULTS")
    print("=" * 50)
...
async def main():
    return await test_amazon_cross_tab_product_comparison()
