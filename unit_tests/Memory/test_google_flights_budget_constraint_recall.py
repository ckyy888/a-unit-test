...
class GoogleFlightsBudgetConstraintRecallTest:
    def __init__(self):
        self.url = "https://example.com/flights/search?budget=300"
        self.first_task_description = (
            """
            Agent 1: Budget limit is $300, and filters are visible for "$300 and under".
            Choose a qualifying flight and proceed to checkout.
            """
        )
        self.second_task_description = (
            """
            Agent 2: Budget limit is $300 but results show all prices without filters.
            Remember the budget and choose a qualifying flight. Proceed to checkout.
            """
        )

    def evaluate_checkout(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        final_url = agent_result.get("final_url", "")
        success_prefix = "https://example.com/flights/checkout"
        passed = final_url.startswith(success_prefix)
        return {
            "passed": passed,
            "reason": (
                "Budget-respecting flight selected and checkout reached"
                if passed
    ...
    async def run_first_test(self, task_description: str) -> Dict[str, Any]:
        ...
        evaluation = self.evaluate_checkout(result)
        return {
            "test_name": "first_test",
            "scenario": "filters_visible",
            "agent_result": result,
    ...
    async def run_second_test(self, task_description: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "scenario": "no_filters_memory_needed",
            "agent_result": result,
            "evaluation": evaluation,
            "passed": evaluation["passed"],
        }
...
async def test_google_flights_budget_constraint_recall():
    test_runner = GoogleFlightsBudgetConstraintRecallTest()
    results = await test_runner.run_both_tests(
        test_runner.first_task_description, test_runner.second_task_description
    )

    print("\n" + "=" * 50)
    print("GOOGLE FLIGHTS BUDGET CONSTRAINT RECALL TEST RESULTS")
    print("=" * 50)
...
async def main():
    return await test_google_flights_budget_constraint_recall()
