...
class ShoppingListSynonymsTest:
    def __init__(self):
        self.url = "https://example.com/groceries/list"
        self.first_task_description = (
            """
            Agent 1: Shopping list contains items like "tomatoes". Purchase the correct items.
            """
        )
        self.second_task_description = (
            """
            Agent 2: Shopping list uses synonyms like "fresh tomatoes" or "red tomatoes".
            Purchase the same set of items.
            """
        )

    def evaluate_checkout(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        final_url = agent_result.get("final_url", "")
        success_prefix = "https://example.com/groceries/checkout"
        passed = final_url.startswith(success_prefix)
        return {
            "passed": passed,
            "reason": (
                "Proceeded to checkout with correct items"
                if passed
...
async def test_shopping_list_synonyms():
    test_runner = ShoppingListSynonymsTest()
    results = await test_runner.run_both_tests(
        test_runner.first_task_description, test_runner.second_task_description
    )

    print("\n" + "=" * 50)
    print("SHOPPING LIST SYNONYMS TEST RESULTS")
    print("=" * 50)
...
async def main():
    return await test_shopping_list_synonyms()
