...
class FormFieldPersistenceTest:
    def __init__(self):
        self.url = "https://example.com/long-form"
        self.first_task_description = (
            """
            Agent 1: Fill a long form. The browser keeps the state when navigating between pages.
            Avoid re-entering previously filled information and proceed to submission.
            """
        )
        self.second_task_description = (
            """
            Agent 2: Fill the same form. The browser loses state when navigating back and forth.
            Reconstruct entries from memory and proceed to submission.
            """
        )

    def evaluate_submission(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        final_url = agent_result.get("final_url", "")
        success_prefix = "https://example.com/long-form/submitted"
        passed = final_url.startswith(success_prefix)
        return {
            "passed": passed,
            "reason": (
                "Form submitted successfully" if passed else f"Unexpected final URL: {final_url}"
            ),
    ...
    async def run_first_test(self, task_description: str) -> Dict[str, Any]:
        ...
        evaluation = self.evaluate_submission(result)
        return {
            "test_name": "first_test",
            "scenario": "state_persisted",
            "agent_result": result,
    ...
    async def run_second_test(self, task_description: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "scenario": "state_lost_reconstruct",
            "agent_result": result,
            "evaluation": evaluation,
            "passed": evaluation["passed"],
        }
...
async def test_form_field_persistence():
    test_runner = FormFieldPersistenceTest()
    results = await test_runner.run_both_tests(
        test_runner.first_task_description, test_runner.second_task_description
    )

    print("\n" + "=" * 50)
    print("FORM FIELD PERSISTENCE TEST RESULTS")
    print("=" * 50)
...
async def main():
    return await test_form_field_persistence()
