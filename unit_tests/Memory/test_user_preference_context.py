...
class UserPreferenceContextTest:
    def __init__(self):
        self.url = "https://example.com/travel/booking"
        self.first_task_description = (
            """
            Agent 1: User preferences (dietary restrictions/accessibility) are mentioned at the start.
            Apply them when booking and proceed to confirmation.
            """
        )
        self.second_task_description = (
            """
            Agent 2: Same preferences appear mid-conversation.
            Apply them consistently and proceed to confirmation.
            """
        )

    def evaluate_confirmation(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        final_url = agent_result.get("final_url", "")
        success_prefix = "https://example.com/travel/booking/confirmation"
        passed = final_url.startswith(success_prefix)
        return {
            "passed": passed,
            "reason": (
                "Booking confirmed with remembered preferences"
                if passed
...
async def test_user_preference_context():
    test_runner = UserPreferenceContextTest()
    results = await test_runner.run_both_tests(
        test_runner.first_task_description, test_runner.second_task_description
    )

    print("\n" + "=" * 50)
    print("USER PREFERENCE CONTEXT TEST RESULTS")
    print("=" * 50)
...
async def main():
    return await test_user_preference_context()
