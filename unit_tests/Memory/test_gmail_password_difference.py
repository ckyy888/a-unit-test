...
class GmailPasswordDifferenceTest:
    def __init__(self):
        self.url = "https://example.com/gmail/login"
        self.first_task_description = (
            """
            Agent 1: Enter the correct remembered password and sign in successfully.
            """
        )
        self.second_task_description = (
            """
            Agent 2: Password prompt wording differs, but enter the same correct password and sign in.
            """
        )

    def evaluate_login(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        final_url = agent_result.get("final_url", "")
        success_prefix = "https://example.com/gmail/inbox"
        passed = final_url.startswith(success_prefix)
        return {
            "passed": passed,
            "reason": (
                "Login succeeded and inbox reached" if passed else f"Unexpected final URL: {final_url}"
            ),
    ...
    async def run_first_test(self, task_description: str) -> Dict[str, Any]:
        ...
        evaluation = self.evaluate_login(result)
        return {
...
async def test_gmail_password_difference():
    test_runner = GmailPasswordDifferenceTest()
    results = await test_runner.run_both_tests(
        test_runner.first_task_description, test_runner.second_task_description
    )

    print("\n" + "=" * 50)
    print("GMAIL PASSWORD DIFFERENCE TEST RESULTS")
    print("=" * 50)
...
async def main():
    return await test_gmail_password_difference()
