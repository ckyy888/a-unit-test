...
from browser_env.actions import create_double_click_action, ActionTypes
...
class EspnDoubleClickValidator:
    ...
    def log_double_click_coordinates_and_timing(self, coordinates: Dict, timing: Dict) -> None:
        ...
        print(f"Double-click coordinates and timing: ({coordinates['x']}, {coordinates['y']}) with {timing['interval']}ms interval")
        self.logger.info(f"Double-click executed at {coordinates} with timing {timing}")
    ...
    def validate_double_click_result(self) -> Dict[str, Any]:
        ...
        if not self.double_click_executed:
            validation_result['errors'].append("Double click action was not executed")
        
        # Check if coordinates were logged
        if not self.click_coordinates:
            validation_result['errors'].append("Double-click coordinates were not logged")
        
        # Check if timing was logged
        if not self.click_timing:
            validation_result['warnings'].append("Double-click timing was not logged")
        
        # Check if team stats expanded
        if not self.team_stats_expanded:
            validation_result['errors'].append("Team stats did not expand below row")
        
        # Determine if double click was successful
        validation_result['double_click_successful'] = (
            self.double_click_executed and 
            self.team_stats_expanded
        )
        
        # Overall validation status
        validation_result['overall_valid'] = (
            validation_result['double_click_successful'] and
...
class EspnDoubleClickTestEnvironment:
    ...
    def __init__(self, url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None):
        ...
        self.espn_elements_config = self.config_loader.get_section('espn_double_click_elements')
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            # Store initial state
            self.test_state['team_row_collapsed_before'] = not standings_state.get('expanded_stats_visible', False)
            ...
            setup_result = {
                ...
                'current_standings': {
                    'team_row_collapsed': self.test_state['team_row_collapsed_before'],
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            click_info = self._extract_double_click_info(action, browser_result)
            if click_info:
                self.validator.log_double_click_coordinates_and_timing(
                    click_info['coordinates'], 
                    click_info['timing']
                )
            ...
            self.test_state['team_row_expanded_after'] = updated_standings_state.get('expanded_stats_visible', False)
            self.test_state['detailed_stats_visible'] = updated_standings_state.get('detailed_stats_count', 0) > 0
            ...
            if action_type == 11:  # DOUBLE_CLICK action
                ...
                if self.test_state['team_row_expanded_after']:
                    evaluation['feedback'] += ' - team stats successfully expanded'
                else:
                    evaluation['feedback'] += ' - team stats expansion not detected'
                
                evaluation['test_completed'] = True
                evaluation['validation_result'] = self.validator.validate_double_click_result()
            else:
                evaluation['feedback'] = f'Unexpected action type {action_type} - expected DOUBLE_CLICK (11) only'
    ...
    def _extract_double_click_info(self, action: Dict, browser_result: Dict) -> Optional[Dict]:
        """Extract double-click coordinates and timing information."""
        # Get coordinates
        if 'coordinates' in action:
            coordinates = action['coordinates']
        else:
            coords = self.espn_elements_config.get('team_row', {}).get('coordinates', {'x': 0.5, 'y': 0.4})
            coordinates = coords
        
        # Get timing information
        timing = {
            'interval': browser_result.get('double_click_interval', 200),  # ms between clicks
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            'team_row_collapsed_before': self.test_state['team_row_collapsed_before'],
            'team_row_expanded_after': self.test_state['team_row_expanded_after'],
            'detailed_stats_visible': self.test_state['detailed_stats_visible'],
...
async def test_espn_double_click_expand(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """
...
def test_espn_double_click_expand_sync(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """Synchronous wrapper for the async test function."""
    return asyncio.run(test_espn_double_click_expand(url, run_agent_func, backend_url, config_path))


# Convenience function for quick integration
def create_espn_double_click_expand_test(url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> EspnDoubleClickTestEnvironment:
