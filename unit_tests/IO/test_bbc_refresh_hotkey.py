...
from browser_env.actions import create_hotkey_action, ActionTypes
...
class BBCRefreshValidator:
    ...
    def validate_refresh_result(self) -> Dict[str, Any]:
        ...
        if not self.hotkey_executed:
            validation_result['errors'].append("Hotkey action was not executed")
        
        # Check if hotkey combination is valid
        if self.hotkey_combination not in ['Ctrl+R', 'F5', 'Command+R']:
            validation_result['warnings'].append(f"Unexpected hotkey combination: {self.hotkey_combination}")
        ...
        if not self.timestamp_updated:
            validation_result['warnings'].append("Article timestamp did not update")
        
        # Determine if refresh was successful
        validation_result['refresh_successful'] = (
            self.hotkey_executed and 
            self.page_reloaded
        )
        
        # Overall validation status
        validation_result['overall_valid'] = (
            validation_result['refresh_successful'] and
...
class BBCRefreshTestEnvironment:
    ...
    def __init__(self, url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None):
        ...
        self.bbc_elements_config = self.config_loader.get_section('bbc_refresh_elements')
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            self.test_state['timestamp_before'] = article_state.get('timestamp_text')
            ...
            self.logger.info(f"BBC refresh hotkey test initialized: {self.url}")
            return setup_result
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            self.test_state['timestamp_after'] = updated_article_state.get('timestamp_text')
            ...
            if action_type == 10:  # HOTKEY action
                ...
                evaluation['validation_result'] = self.validator.validate_refresh_result()
            else:
                evaluation['feedback'] = f'Unexpected action type {action_type} - expected HOTKEY (10) only'
...
async def test_bbc_refresh_hotkey(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """
...
def test_bbc_refresh_hotkey_sync(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """Synchronous wrapper for the async test function."""
    return asyncio.run(test_bbc_refresh_hotkey(url, run_agent_func, backend_url, config_path))


# Convenience function for quick integration
def create_bbc_refresh_hotkey_test(url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> BBCRefreshTestEnvironment:
