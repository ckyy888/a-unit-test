...
class AmazonSingleClickValidator:
    ...
    def log_exact_click_coordinates(self, coordinates: Dict) -> None:
        ...
        print(
            f"Exact click coordinates: x={coordinates['x']}, y={coordinates['y']}"
        )
        self.logger.info(
            f"Click executed at precise coordinates: {coordinates}"
        )
    ...
    def validate_single_click_result(self) -> Dict[str, Any]:
        ...
        if not self.exact_coordinates:
            validation_result["errors"].append(
                "Exact click coordinates were not logged"
            )
        ...
        if not self.button_feedback_appeared:
            validation_result["warnings"].append(
                "Button feedback did not appear"
            )

        # Determine if single click was successful
        validation_result["single_click_successful"] = (
            self.click_executed and self.exact_coordinates is not None
        )
...
class AmazonSingleClickTestEnvironment:
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            self.test_state["cart_count_before"] = cart_state.get(
                "cart_count", 0
            )
            ...
            setup_result = {
                ...
                "current_cart": {
                    ...
                    "add_to_cart_visible": element_detection.get(
                        "add_to_cart_button", {}
                    ).get("found", False),
                },
                "current_step": 0,
            ...
            self.logger.info(
                f"Amazon single click test initialized: {self.url}"
            )
            return setup_result
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            if "coordinates" in action:
                self.validator.log_exact_click_coordinates(
                    action["coordinates"]
                )
            ...
            self.test_state["cart_count_after"] = updated_cart_state.get(
                "cart_count", 0
            )
            ...
            if action_type == 6:  # CLICK action
                if "add" in element_name and "cart" in element_name:
                    ...
                    # Check if cart counter incremented
                    if (
                        self.test_state["cart_count_after"]
                        > self.test_state["cart_count_before"]
                    ):
                        self.validator.cart_counter_incremented = True
                        print(
                            f"Cart counter incremented: {self.test_state['cart_count_before']} → {self.test_state['cart_count_after']}"
                        )
                    ...
                    if updated_cart_state.get("button_feedback_visible"):
                        ...
                        print(
                            "Button feedback appears: Add to Cart success indication visible"
                        )

                    evaluation["test_completed"] = True
                    evaluation[
                        "validation_result"
                    ] = self.validator.validate_single_click_result()
                else:
                    evaluation[
                        "feedback"
                    ] = f"Clicked wrong element - expected Add to Cart button, got: {element_name}"
            else:
                evaluation[
                    "feedback"
                ] = f"Unexpected action type {action_type} - expected CLICK (6) only"
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            "button_feedback_visible": self.test_state[
                "button_feedback_visible"
            ],
...
async def test_amazon_single_click(
    url: str,
...
def test_amazon_single_click_sync(
    url: str,
    run_agent_func: Callable,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
    ...
    return asyncio.run(
        test_amazon_single_click(url, run_agent_func, backend_url, config_path)
    )


# Convenience function for quick integration
def create_amazon_single_click_test(
    url: str,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
) -> AmazonSingleClickTestEnvironment:
