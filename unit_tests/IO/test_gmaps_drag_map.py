...
from browser_env.actions import create_drag_action, ActionTypes
...
class GoogleMapsDragValidator:
    ...
    def log_drag_vector_and_distance(self, vector: Dict, distance: float) -> None:
        """Print drag vector and distance."""
        self.drag_vector = vector
        self.drag_distance = distance
        self.drag_executed = True
        print(f"Drag vector and distance: from ({vector['from_x']}, {vector['from_y']}) to ({vector['to_x']}, {vector['to_y']}) - {distance}px")
    ...
    def validate_drag_result(self) -> Dict[str, Any]:
        ...
        if self.drag_distance < 150:
            validation_result['warnings'].append(f"Drag distance may be insufficient: {self.drag_distance}px")
        elif self.drag_distance > 300:
            validation_result['warnings'].append(f"Drag distance may be excessive: {self.drag_distance}px")
        
        # Check if map center shifted
        if not self.map_center_shifted:
            validation_result['errors'].append("Map center did not shift eastward")
        
        # Check if new area is visible
        if not self.new_area_visible:
            validation_result['warnings'].append("New area may not be visible after drag")
        
        # Determine if drag was successful
        validation_result['drag_successful'] = (
            self.drag_executed and 
            self.map_center_shifted
        )
        
        # Overall validation status
        validation_result['overall_valid'] = (
            validation_result['drag_successful'] and
...
class GoogleMapsDragTestEnvironment:
    ...
    def __init__(self, url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None):
        ...
        self.gmaps_elements_config = self.config_loader.get_section('gmaps_drag_elements')
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"Google Maps drag test initialized: {self.url}")
            return setup_result
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            if drag_info:
                self.validator.log_drag_vector_and_distance(drag_info['vector'], drag_info['distance'])
                self.test_state['drag_start_coords'] = drag_info['vector']['from']
            ...
            self.test_state['map_center_after'] = updated_map_state.get('map_center')
            ...
            if action_type == 8:  # DRAG action
                ...
                evaluation['feedback'] = f'Drag executed - distance: {drag_info["distance"] if drag_info else "unknown"}px'
                self.test_state['drag_executed'] = True
                
                if self.test_state['map_view_changed']:
                    print(f"Map center shifts eastward, new area visible")
                    evaluation['feedback'] += ' - map view successfully shifted'
                else:
                    evaluation['feedback'] += ' - map view may not have shifted'
                
                evaluation['test_completed'] = True
                evaluation['validation_result'] = self.validator.validate_drag_result()
            else:
                evaluation['feedback'] = f'Unexpected action type {action_type} - expected DRAG (8) only'
    ...
    def _extract_drag_info(self, action: Dict, browser_result: Dict) -> Optional[Dict]:
        """Extract drag vector and distance information."""
        # Default coordinates from config
        center_coords = self.gmaps_elements_config.get('map_center', {}).get('coordinates', {'x': 0.5, 'y': 0.5})
        target_coords = self.gmaps_elements_config.get('drag_target', {}).get('coordinates', {'x': 0.7, 'y': 0.5})
        ...
        elif 'coordinate' in action and isinstance(action['coordinate'], list) and len(action['coordinate']) >= 4:
    ...
    async def _execute_action_in_browser(self, action: Dict) -> Dict:
        ...
        if action_type == 8:  # DRAG action
            ...
            try:
                async with self.http_session.post(f"{self.backend_url}/drag", json=payload) as response:
                    result = await response.json()
                    self.logger.info(f"Drag executed from ({start_x}, {start_y}) to ({end_x}, {end_y})")
                    return result
            except Exception as e:
...
async def test_gmaps_drag_map(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """
...
def test_gmaps_drag_map_sync(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """Synchronous wrapper for the async test function."""
    return asyncio.run(test_gmaps_drag_map(url, run_agent_func, backend_url, config_path))


# Convenience function for quick integration
def create_gmaps_drag_map_test(url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> GoogleMapsDragTestEnvironment:
