...
from browser_env.actions import ActionTypes, create_move_mouse_action
...
class AmazonMouseHoverValidator:
    ...
    def log_mouse_hover_coordinates(self, coordinates: Dict) -> None:
        ...
        print(
            f"Mouse hover coordinates: ({coordinates['x']}, {coordinates['y']})"
        )
...
class AmazonMouseHoverTestEnvironment:
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            # Store initial state
            self.test_state["product_cards_found"] = product_detection.get(
                "product_card", {}
            ).get("found", False)
            self.test_state["preview_visible_before"] = preview_state.get(
            ...
            setup_result = {
                ...
                "current_product_grid": {
                    "product_cards_found": self.test_state[
                        "product_cards_found"
                    ],
                    "preview_visible": self.test_state[
                        "preview_visible_before"
                    ],
            ...
            self.logger.info(
                f"Amazon mouse hover test initialized: {self.url}"
            )
            return setup_result
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            if "coordinates" in action:
                self.validator.log_mouse_hover_coordinates(
                    action["coordinates"]
                )
                self.test_state["hover_coordinates"] = action["coordinates"]
            elif browser_result and "hover_coordinates" in browser_result:
                self.validator.log_mouse_hover_coordinates(
                    browser_result["hover_coordinates"]
                )
                self.test_state["hover_coordinates"] = browser_result[
                    "hover_coordinates"
                ]
            ...
            ] = updated_preview_state.get("preview_visible", False)
            self.test_state[
                "quick_view_popup_visible"
            ] = updated_preview_state.get("quick_view_visible", False)
            ...
            if action_type == 14:  # MOVE_MOUSE action
                ...
                if self.test_state["preview_visible_after"]:
                    evaluation[
                        "feedback"
                    ] += " - product preview successfully appeared"
                    if self.test_state["quick_view_popup_visible"]:
                        evaluation["feedback"] += " with quick view popup"
                else:
                    evaluation[
                        "feedback"
                    ] += " - product preview did not appear"

                evaluation["test_completed"] = True
                evaluation[
                    "validation_result"
                ] = self.validator.validate_mouse_hover_result()
            else:
                evaluation[
                    "feedback"
                ] = f"Unexpected action type {action_type} - expected MOVE_MOUSE (14) only"
    ...
    async def _execute_action_in_browser(self, action: Dict) -> Dict:
        ...
        if action_type == 14:  # MOVE_MOUSE action
            ...
            if "coordinates" in action:
                ...
                coords = self.amazon_elements_config.get(
                    "product_card", {}
                ).get("coordinates", {"x": 0.3, "y": 0.4})
                x, y = coords["x"], coords["y"]

            # Execute mouse move via browser backend
            payload = {"x": x, "y": y, "action": "move"}

            try:
                async with self.http_session.post(
                    f"{self.backend_url}/move_mouse", json=payload
                ) as response:
                    result = await response.json()
                ...
                try:
                    ...
                    async with self.http_session.post(
                        ...
                        result["hover_coordinates"] = {"x": x, "y": y}
                        self.logger.info(
                            f"Mouse moved to ({x}, {y}) via fallback"
                        )
                        return result
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            "preview_visible_before": self.test_state[
                "preview_visible_before"
            ],
            "preview_visible_after": self.test_state["preview_visible_after"],
            "quick_view_popup_visible": self.test_state[
                "quick_view_popup_visible"
            ],
...
async def test_amazon_mouse_hover_preview(
    url: str,
...
def test_amazon_mouse_hover_preview_sync(
    url: str,
    run_agent_func: Callable,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
    ...
    return asyncio.run(
        test_amazon_mouse_hover_preview(
...
def create_amazon_mouse_hover_preview_test(
    url: str,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
) -> AmazonMouseHoverTestEnvironment:
