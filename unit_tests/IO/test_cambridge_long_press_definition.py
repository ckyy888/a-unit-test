...
class CambridgeLongPressValidator:
    ...
    def log_long_press_duration_and_coordinates(
        self, duration: int, coordinates: Dict
    ) -> None:
        ...
        print(
            f"Long press duration and coordinates: {duration}ms at ({coordinates['x']}, {coordinates['y']})"
        )
...
class CambridgeLongPressTestEnvironment:
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            # Store initial state
            self.test_state["word_coordinates"] = word_detection.get(
                "word_ubiquitous", {}
            ).get("coordinates")
            self.test_state["definition_tooltip_before"] = tooltip_state.get(
            ...
            setup_result = {
                ...
                "current_dictionary": {
                    ...
                    "definition_tooltip_visible": self.test_state[
                        "definition_tooltip_before"
                    ],
    ...
    async def _detect_word_elements(self) -> Dict:
        ...
        tooltip_config = self.cambridge_elements_config.get(
            "definition_tooltip", {}
        )
        detected = {
            "found": False,
        ...
        try:
            ...
            if press_info:
                self.validator.log_long_press_duration_and_coordinates(
                    press_info["duration"], press_info["coordinates"]
                )
            ...
            self.test_state[
                "definition_tooltip_after"
            ] = updated_tooltip_state.get("tooltip_visible", False)
            self.test_state["tooltip_content"] = updated_tooltip_state.get(
                "tooltip_content", ""
            )
            ...
            if action_type == 13:  # LONG_PRESS action
                ...
                evaluation[
                    "feedback"
                ] = f'Long press executed - duration: {press_info["duration"] if press_info else "unknown"}ms'
                self.test_state["long_press_executed"] = True

                if self.test_state["definition_tooltip_after"]:
                    evaluation[
                        "feedback"
                    ] += " - inline definition tooltip successfully appeared"
                else:
                    evaluation[
                        "feedback"
                    ] += " - inline definition tooltip did not appear"
                ...
                evaluation[
                    "validation_result"
                ] = self.validator.validate_long_press_result()
            else:
                evaluation[
                    "feedback"
                ] = f"Unexpected action type {action_type} - expected LONG_PRESS (13) only"
    ...
    def _extract_long_press_info(
        ...
        if "coordinates" in action:
            ...
            coordinates = self.cambridge_elements_config.get(
                "word_ubiquitous", {}
            ).get("coordinates", {"x": 0.4, "y": 0.5})
    ...
    async def _execute_action_in_browser(self, action: Dict) -> Dict:
        ...
        if action_type == 13:  # LONG_PRESS action
            ...
            try:
                ...
                # Mouse down
                async with self.http_session.post(
                    f"{self.backend_url}/click", json=payload_down
                ) as response:
                    await response.json()
                ...
                # Mouse up
                async with self.http_session.post(
                    f"{self.backend_url}/click", json=payload_up
                ) as response:
                    result = await response.json()
                ...
                self.logger.info(
                    f"Long press executed at ({x}, {y}) for {actual_duration}ms"
                )
                return result
            except Exception as e:
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            "word_coordinates": self.test_state["word_coordinates"],
            "definition_tooltip_before": self.test_state[
                "definition_tooltip_before"
            ],
            "definition_tooltip_after": self.test_state[
                "definition_tooltip_after"
            ],
...
async def test_cambridge_long_press_definition(
    url: str,
...
def test_cambridge_long_press_definition_sync(
    url: str,
    run_agent_func: Callable,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
    ...
    return asyncio.run(
        test_cambridge_long_press_definition(
...
def create_cambridge_long_press_definition_test(
    url: str,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
) -> CambridgeLongPressTestEnvironment:
