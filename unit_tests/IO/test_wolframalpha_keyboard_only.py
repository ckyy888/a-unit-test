...
from browser_env.actions import ActionTypes, create_keyboard_type_action
    ...
    def validate_keyboard_result(self) -> Dict[str, Any]:
        ...
        if not self.text_entered:
            validation_result["errors"].append(
                "Keyboard input was not executed"
            )

        # Check if exact text was logged
        if not self.exact_text:
            validation_result["errors"].append(
                "Exact text entered was not logged"
            )
...
class WolframAlphaKeyboardTestEnvironment:
    ...
    def __init__(
        ...
        self.wolfram_elements_config = self.config_loader.get_section(
            "wolframalpha_keyboard_elements"
        )
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            self.test_state["query_box_focused"] = input_state.get(
                "focused", False
            )
            self.test_state["query_box_empty_before"] = input_state.get(
                "empty", True
            )
            ...
            self.logger.info(
                f"WolframAlpha keyboard test initialized: {self.url}"
            )
            return setup_result
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            # Stabilise with stored metadata if present; otherwise fetch fresh
            ...
            self.test_state["query_box_contains_text_after"] = (
                "2+2" in current_value
            )
            ...
            if action_type == 3:  # TYPE action
                ...
                evaluation[
                    "feedback"
                ] = f'Keyboard input executed: "{text_entered}"'
                ...
                if self.test_state["query_box_contains_text_after"]:
                    ...
                    evaluation[
                        "feedback"
                    ] += " - query box successfully updated"
                else:
                    evaluation[
                        "feedback"
                    ] += f' - query box does not contain expected text (got: "{current_value}")'

                evaluation["test_completed"] = True
                evaluation[
                    "validation_result"
                ] = self.validator.validate_keyboard_result()
            else:
                evaluation[
                    "feedback"
                ] = f"Unexpected action type {action_type} - expected TYPE (3) only"
    ...
    async def _execute_action_in_browser(self, action: Dict, obs_metadata: Dict = None) -> Dict:
        ...
        try:
            ...
            async with self.http_session.post(
                ...
                result = await response.json()
                if route == "/keyboard":
                    self.logger.info(f"Keyboard input executed: '{payload.get('key', '')}'")
                return result
        except Exception as e:
            self.logger.warning(f"Type action conversion failed, using fallback: {e}")

        if action_type == 3:  # TYPE action fallback
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            "query_box_empty_before": self.test_state[
                "query_box_empty_before"
            ],
            "query_box_contains_text_after": self.test_state[
                "query_box_contains_text_after"
            ],
    ...
    def save_full_trajectory(self) -> None:
        ...
        try:
            trajectory_file = Path("/home/ubuntu/webarena/unit_tests/IO/wolfram_keyboard_trajectory.json")
    ...
    def save_workflow_summary(self) -> None:
        ...
        try:
            summary = {
                ...
                "test_completed": self.test_state["keyboard_executed"],
                "text_entered": self.validator.exact_text,
                "final_validation": self.validator.validate_keyboard_result(),
            ...
            summary_file = Path("/home/ubuntu/webarena/unit_tests/IO/wolfram_keyboard_summary.json")
...
async def test_wolframalpha_keyboard_only(
    url: str,
    ...
    """
    test_env = WolframAlphaKeyboardTestEnvironment(
...
def test_wolframalpha_keyboard_only_sync(
    url: str,
    run_agent_func: Callable,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
    ...
    return asyncio.run(
        test_wolframalpha_keyboard_only(
...
def create_wolframalpha_keyboard_only_test(
    url: str,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
) -> WolframAlphaKeyboardTestEnvironment:
