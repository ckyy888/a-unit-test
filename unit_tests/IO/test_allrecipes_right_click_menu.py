...
from browser_env.actions import create_right_click_action, ActionTypes
...
class AllRecipesRightClickValidator:
    ...
    def log_right_click_coordinates(self, coordinates: Dict) -> None:
        ...
        print(f"Right-click coordinates: ({coordinates['x']}, {coordinates['y']})")
    ...
    def validate_right_click_result(self) -> Dict[str, Any]:
        ...
        if not self.right_click_executed:
            validation_result['errors'].append("Right click action was not executed")
        
        # Check if coordinates were logged
        if not self.right_click_coordinates:
            validation_result['errors'].append("Right-click coordinates were not logged")
        ...
        if not self.save_image_option_found:
            validation_result['warnings'].append("'Save image' option not found in context menu")
        
        # Determine if right click was successful
        validation_result['right_click_successful'] = (
            self.right_click_executed and 
            self.context_menu_appeared
        )
        
        # Overall validation status
        validation_result['overall_valid'] = (
            validation_result['right_click_successful'] and
...
class AllRecipesRightClickTestEnvironment:
    ...
    def __init__(self, url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None):
        ...
        self.allrecipes_elements_config = self.config_loader.get_section('allrecipes_right_click_elements')
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            # Store initial state
            self.test_state['recipe_image_found'] = element_detection.get('recipe_image', {}).get('found', False)
            self.test_state['context_menu_visible_before'] = menu_state.get('context_menu_visible', False)
            ...
            setup_result = {
                ...
                'current_recipe_grid': {
                    ...
                    'context_menu_visible': self.test_state['context_menu_visible_before'],
            ...
            self.logger.info(f"AllRecipes right click test initialized: {self.url}")
            return setup_result
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            if 'coordinates' in action:
                self.validator.log_right_click_coordinates(action['coordinates'])
            elif browser_result and 'click_coordinates' in browser_result:
                self.validator.log_right_click_coordinates(browser_result['click_coordinates'])
            ...
            self.test_state['context_menu_visible_after'] = updated_menu_state.get('context_menu_visible', False)
            self.test_state['save_image_option_present'] = updated_menu_state.get('save_image_option_present', False)
            ...
            if action_type == 12:  # RIGHT_CLICK action
                if 'recipe' in element_name and 'image' in element_name:
                    ...
                    evaluation['feedback'] = 'Right-click executed on recipe image'
                    self.test_state['right_click_executed'] = True
                    
                    if self.test_state['context_menu_visible_after']:
                        evaluation['feedback'] += ' - context menu successfully appeared'
                        if self.test_state['save_image_option_present']:
                            evaluation['feedback'] += ' with "Save image" option'
                    else:
                        evaluation['feedback'] += ' - context menu did not appear'
                    
                    evaluation['test_completed'] = True
                    evaluation['validation_result'] = self.validator.validate_right_click_result()
                else:
                    evaluation['feedback'] = f'Right-clicked wrong element - expected recipe image, got: {element_name}'
            else:
                evaluation['feedback'] = f'Unexpected action type {action_type} - expected RIGHT_CLICK (12) only'
    ...
    async def _execute_action_in_browser(self, action: Dict) -> Dict:
        ...
        if action_type == 12:  # RIGHT_CLICK action
            ...
            if 'coordinates' in action:
                ...
                coords = self.allrecipes_elements_config.get('recipe_image', {}).get('coordinates', {'x': 0.3, 'y': 0.6})
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            'recipe_image_found': self.test_state['recipe_image_found'],
            'context_menu_visible_before': self.test_state['context_menu_visible_before'],
            'context_menu_visible_after': self.test_state['context_menu_visible_after'],
            'save_image_option_present': self.test_state['save_image_option_present'],
...
async def test_allrecipes_right_click_menu(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """
    Test function for AllRecipes right click menu with real browser integration.
    """
    test_env = AllRecipesRightClickTestEnvironment(url, backend_url, config_path)
...
def test_allrecipes_right_click_menu_sync(url: str, run_agent_func: Callable, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> Dict:
    """Synchronous wrapper for the async test function."""
    return asyncio.run(test_allrecipes_right_click_menu(url, run_agent_func, backend_url, config_path))


# Convenience function for quick integration
def create_allrecipes_right_click_menu_test(url: str, backend_url: Optional[str] = None, config_path: Optional[str] = None) -> AllRecipesRightClickTestEnvironment:
