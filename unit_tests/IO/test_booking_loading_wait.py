...
from browser_env.actions import ActionTypes, create_wait_action
    ...
    def validate_wait_result(self) -> Dict[str, Any]:
        ...
        if self.wait_duration < 2000:
            validation_result["warnings"].append(
                f"Wait duration may be insufficient: {self.wait_duration}ms"
            )
        elif self.wait_duration > 5000:
            validation_result["warnings"].append(
                f"Wait duration may be excessive: {self.wait_duration}ms"
            )
        ...
        validation_result["loading_completed"] = (
            self.spinner_disappeared and self.hotel_results_appeared
        )

        # Determine if wait was successful
        validation_result["wait_successful"] = (
            self.wait_executed and validation_result["loading_completed"]
        )
...
class BookingWaitTestEnvironment:
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            self.test_state["spinner_visible_before"] = loading_state.get(
                "spinner_visible", True
            )
            self.test_state["results_visible_before"] = loading_state.get(
            ...
            setup_result = {
                ...
                "current_loading": {
                    "spinner_visible": self.test_state[
                        "spinner_visible_before"
                    ],
                    "results_visible": self.test_state[
                        "results_visible_before"
                    ],
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            self.test_state[
                "spinner_visible_after"
            ] = updated_loading_state.get("spinner_visible", False)
            self.test_state[
                "results_visible_after"
            ] = updated_loading_state.get("results_visible", False)
            self.test_state["loading_completed"] = updated_loading_state.get(
                "loading_complete", False
            )
            ...
            if action_type == 9:  # WAIT action
                ...
                evaluation[
                    "feedback"
                ] = f"Wait executed - duration: {wait_duration}ms"
                ...
                if self.test_state["results_visible_after"]:
                    ...
                    evaluation[
                        "feedback"
                    ] += " - hotel results successfully loaded"
                else:
                    evaluation[
                        "feedback"
                    ] += " - hotel results not yet visible"

                evaluation["test_completed"] = True
                evaluation[
                    "validation_result"
                ] = self.validator.validate_wait_result()
            else:
                evaluation[
                    "feedback"
                ] = f"Unexpected action type {action_type} - expected WAIT (9) only"
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            "spinner_visible_before": self.test_state[
                "spinner_visible_before"
            ],
            "spinner_visible_after": self.test_state["spinner_visible_after"],
            "results_visible_before": self.test_state[
                "results_visible_before"
            ],
...
async def test_booking_loading_wait(
    url: str,
...
def test_booking_loading_wait_sync(
    url: str,
    run_agent_func: Callable,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
...
def create_booking_loading_wait_test(
    url: str,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
) -> BookingWaitTestEnvironment:
