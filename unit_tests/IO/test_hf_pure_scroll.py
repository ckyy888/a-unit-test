...
from browser_env.actions import ActionTypes, create_scroll_action
    ...
    def validate_scroll_result(self) -> Dict[str, Any]:
        ...
        if not self.scroll_executed:
            validation_result["errors"].append(
                "Scroll action was not executed"
            )

        # Check if scroll distance was appropriate
        if self.scroll_distance < 500:
            validation_result["warnings"].append(
                f"Scroll distance may be insufficient: {self.scroll_distance}px"
            )
        elif self.scroll_distance > 800:
            validation_result["warnings"].append(
                f"Scroll distance may be excessive: {self.scroll_distance}px"
            )

        # Check if downloads chart is now visible
        if not self.downloads_chart_visible:
            validation_result["errors"].append(
                "Downloads chart is not visible in viewport after scroll"
            )

        # Determine if scroll was successful
        validation_result["scroll_successful"] = (
            self.scroll_executed and self.downloads_chart_visible
        )
...
class HuggingFaceScrollTestEnvironment:
    ...
    def __init__(
        ...
        self.hf_elements_config = self.config_loader.get_section(
            "hf_scroll_elements"
        )
    ...
    async def get_initial_observation(self) -> Dict:
        ...
        try:
            ...
            self.test_state["initial_scroll_position"] = page_state.get(
                "scroll_position", 0
            )
            self.test_state[
                "downloads_chart_before_scroll"
            ] = chart_detection.get("found", False)
            ...
            setup_result = {
                ...
                "current_viewport": {
                    "scroll_position": self.test_state[
                        "initial_scroll_position"
                    ],
                    "downloads_chart_visible": self.test_state[
                        "downloads_chart_before_scroll"
                    ],
            ...
            self.logger.info(
                f"HuggingFace scroll test initialized: {self.url}"
            )
            return setup_result
    ...
    async def evaluate_agent_action(self, action: Dict) -> Dict:
        ...
        try:
            ...
            # Log scroll distance
            scroll_distance = self._extract_scroll_distance(
            ...
            self.test_state["final_scroll_position"] = updated_page_state.get(
                "scroll_position", 0
            )
            self.test_state[
                "downloads_chart_after_scroll"
            ] = updated_chart_detection.get("in_viewport", False)
            ...
            if action_type == 7:  # SCROLL action
                ...
                evaluation[
                    "feedback"
                ] = f"Scroll executed - distance: {scroll_distance}px"
                ...
                if self.test_state["downloads_chart_after_scroll"]:
                    ...
                    evaluation[
                        "feedback"
                    ] += " - downloads chart successfully revealed"
                else:
                    evaluation[
                        "feedback"
                    ] += " - downloads chart still not visible"

                evaluation["test_completed"] = True
                evaluation[
                    "validation_result"
                ] = self.validator.validate_scroll_result()
            else:
                evaluation[
                    "feedback"
                ] = f"Unexpected action type {action_type} - expected SCROLL (7) only"
    ...
    def _extract_scroll_distance(
        ...
        if (
            ...
            and len(action["coordinate"]) >= 2
        ):
            return abs(
                action["coordinate"][1]
            )  # y-coordinate for vertical scroll
        ...
        position_change = (
            self.test_state["final_scroll_position"]
            - self.test_state["initial_scroll_position"]
        )
        ...
        if action_type == 7:  # SCROLL action
            ...
            try:
                async with self.http_session.post(
                    ...
                    self.logger.info(
                        f"Scroll executed: {scroll_distance}px down"
                    )
                    return result
            except Exception as e:
    ...
    def get_test_status(self) -> Dict:
        ...
        return {
            ...
            "initial_scroll_position": self.test_state[
                "initial_scroll_position"
            ],
            "final_scroll_position": self.test_state["final_scroll_position"],
            "downloads_chart_before_scroll": self.test_state[
                "downloads_chart_before_scroll"
            ],
            "downloads_chart_after_scroll": self.test_state[
                "downloads_chart_after_scroll"
            ],
...
def test_hf_pure_scroll_sync(
    ...
    return asyncio.run(
        test_hf_pure_scroll(url, run_agent_func, backend_url, config_path)
    )


# Convenience function for quick integration
def create_hf_pure_scroll_test(
    url: str,
    backend_url: Optional[str] = None,
    config_path: Optional[str] = None,
) -> HuggingFaceScrollTestEnvironment:
